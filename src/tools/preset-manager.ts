...
export function createPresetManager(ctx: PluginInput, config: PluginConfig) {
    ...
    if (!preset) {
      ...
      const hint =
        available.length > 0
          ...
          : 'No presets configured. Define presets in not-my-opencode.jsonc.';
      output.parts.push(
        createInternalAgentTextPart(
  ...
  function formatPresetList(presets: Record<string, Preset>): string {
    ...
    if (names.length === 0) {
      return 'No presets configured. Define presets in not-my-opencode.jsonc under the "presets" field.';
    }
