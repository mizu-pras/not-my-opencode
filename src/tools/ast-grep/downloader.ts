...
export function getCacheDir(): string {
  if (process.platform === 'win32') {
    ...
    return join(base, 'not-my-opencode', 'bin');
  }
...
export async function downloadAstGrep(
  ...
  if (!platformInfo) {
    console.error(
      `[not-my-opencode] Unsupported platform for ast-grep: ${platformKey}`,
    );
    return null;
  }
  ...
  console.log(`[not-my-opencode] Downloading ast-grep binary...`);
  ...
  try {
    ...
    console.log(`[not-my-opencode] ast-grep binary ready.`);

    return binaryPath;
  } catch (err) {
    console.error(
      `[not-my-opencode] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`,
    );
    return null;
  }
}
