...
- `src/tui.ts` adds a separate host TUI module that registers sidebar content showing plugin version, config warning state, and the currently known model for each visible agent.
- `src/tui-state.ts` persists a small XDG-backed snapshot (`agentModels`, `updatedAt`, schema version) used as the handoff boundary between the plugin runtime and the TUI renderer.
...
- TUI integration is intentionally file-backed instead of process-coupled: plugin code writes snapshot updates opportunistically, and the TUI module polls once per second, tolerating missing/invalid state by falling back to defaults.
...
- Plugin registration: `index.ts` merges/overlays agent configs into OpenCode's config, records initial TUI agent-model state, registers tools (`council`, `webfetch`, `ast_grep_*`, todo tools, `subtask`, `read_session`), MCPs (`createBuiltinMcps`), and all hook handlers (`event`, `tool.execute.before/after`, `experimental.chat.system/messages.transform`, `command.execute.before`, etc.).
...
- Runtime event flow also watches `message.updated` so resolved provider/model pairs can refresh per-agent TUI snapshot entries.
...
- TUI flow: `tui.ts` reads package version + local config validity, polls `readTuiSnapshotAsync()`, and renders sidebar rows from snapshot models or fallback agent names.
...
- Connects to `@opencode-ai/plugin/tui` through `src/tui.ts`, using `@opentui/solid` primitives directly rather than React-style wrappers.
...
- Utility integration is visible at runtime through `utils/session-manager.ts` + `utils/task.ts` (task resume support), `utils/system-collapse.ts` (system message normalization), `tui-state.ts` (UI snapshot handoff), and legacy utility support (`logger`, `env`, `polling`, `session`, etc.).
