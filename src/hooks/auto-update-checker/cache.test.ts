  ...
  describe('resolveInstallContext', () => {
    test('detects OpenCode packages install root from runtime package path', async () => {
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          p ===
          '/home/user/.cache/opencode/packages/not-my-opencode@latest/package.json',
      );
      ...
      const context = resolveInstallContext(
        '/home/user/.cache/opencode/packages/not-my-opencode@latest/node_modules/not-my-opencode/package.json',
      );

      expect(context).toEqual({
        installDir:
          '/home/user/.cache/opencode/packages/not-my-opencode@latest',
        packageJsonPath:
          '/home/user/.cache/opencode/packages/not-my-opencode@latest/package.json',
      });
      ...
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          p ===
            '/home/user/.cache/opencode/packages/not-my-opencode@latest/package.json' ||
          p ===
            '/home/user/.cache/opencode/packages/not-my-opencode@latest/node_modules/not-my-opencode',
      );
    ...
    test('keeps working when dependency is already on target version', async () => {
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          ...
          p.endsWith('/.cache/opencode/node_modules/not-my-opencode'),
      );
      const readSpy = spyOn(fs, 'readFileSync').mockReturnValue(
        JSON.stringify({
          dependencies: {
            'not-my-opencode': '1.0.1',
          },
        }),
      );
      const writeSpy = spyOn(fs, 'writeFileSync').mockImplementation(() => {});
      ...
      const result = preparePackageUpdate('1.0.1', 'not-my-opencode', null);
