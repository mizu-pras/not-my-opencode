...
describe('config-io', () => {
  ...
  function writePackageJson(dir: string): void {
    ...
    writeFileSync(
      ...
      JSON.stringify({ name: 'not-my-opencode' }),
    );
  }
  ...
  test('addPluginToOpenCodeConfig adds plugin and removes duplicates', async () => {
    ...
    writeFileSync(
      configPath,
      JSON.stringify({ plugin: ['other', 'not-my-opencode@1.0.0'] }),
    );
    process.argv[1] = '';
    ...
    expect(saved.plugin).toContain('not-my-opencode');
    expect(saved.plugin).not.toContain('not-my-opencode@1.0.0');
  ...
  test('addPluginToOpenCodeConfig stores package name for bunx temp paths', async () => {
    ...
    const packageRoot = join(
      tmpDir,
      'bunx-1000-not-my-opencode@latest',
      'node_modules',
      'not-my-opencode',
    );
    paths.ensureConfigDir();
    ...
    expect(result.success).toBe(true);
    const saved = JSON.parse(readFileSync(configPath, 'utf-8'));
    expect(saved.plugin).toEqual(['not-my-opencode']);
  });
  ...
  test('addPluginToOpenCodeConfig preserves non-string plugin entries when refreshing', async () => {
    ...
    writeFileSync(
      configPath,
      JSON.stringify({
        plugin: ['other-plugin', objectPlugin, 'not-my-opencode@1.0.0'],
      }),
    );
  ...
  test('addPluginToOpenCodeConfig removes tuple plugin entries', async () => {
    ...
    writeFileSync(
      configPath,
      JSON.stringify({
        plugin: ['other', ['not-my-opencode', { enabled: true }]],
      }),
    );
    process.argv[1] = '';
    ...
    const saved = JSON.parse(readFileSync(configPath, 'utf-8'));
    expect(saved.plugin).toEqual(['other', 'not-my-opencode']);
  });
  ...
  test('addPluginToOpenCodeTuiConfig does not bypass OPENCODE_TUI_CONFIG for existing default config', async () => {
    ...
    expect(custom.plugin).toEqual(['not-my-opencode']);
  ...
  test('writeLiteConfig writes lite config with OpenAI preset', () => {
    const litePath = join(tmpDir, 'opencode', 'not-my-opencode.json');
    paths.ensureConfigDir();
